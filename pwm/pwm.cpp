	...
	*/
	...
	: m_port ( puerto ), m_pin (bit) , m_activity ( actividad ) , m_pwm_channel ( number )
	{
		m_ton = 0;
		m_toff = 0;
	}
	...
	*/
	void Pwm::SetTon( uint32_t time , pwm_time_unit_t t )
	{
		...
		SetTime(m_ton * (FREQ_CLOCK / 1000000), m_pwm_channel);
	}
	/**
	...
	*/
	void Pwm::SetPeriod( uint32_t time , pwm_time_unit_t t)
	{
		...
		SetTime((m_ton + m_toff) * (FREQ_CLOCK / 1000000), 0);
	}
	/**
	...
	*/
	...
	{
		SetSwitchMatrizSCTOUT( m_pin , m_port , m_pwm_channel - 1 );
		...
		SetTime((m_ton + m_toff) * (FREQ_CLOCK / 1000000), 0);	/*	CHANNEL 0 ALWAYS IS THE PERIOD	*/
		SetTime( m_ton * (FREQ_CLOCK / 1000000), m_pwm_channel);	/*	Setteo del tiempo de encendido	*/
